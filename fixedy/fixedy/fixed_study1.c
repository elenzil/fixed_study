...
s1 addSat(const s1 a, const s1 b) {
    bool negA   = a < 0;
    bool negB   = b < 0;
    bool hetero = negA != negB;

    if (hetero) {
        // no overflow with a negative and a positive.
        return a + b;
    }
...
s2 s2_max = { .hi =  127, .lo = 255 };
s2 s2_min = { .hi = -128, .lo = 255 };
